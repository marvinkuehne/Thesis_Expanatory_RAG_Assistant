...
Optional user friendly business name of the folder. If set, this
...
reference: https://docs.oracle.com/en-us/iaas/api/#/en/datacatalog/20
190325/datatypes/CustomPropertySetUsage. This is a complex type whose
...
The job key of the harvest process that updated the folder definition
...
The key of the containing folder or null if there isn’t a parent
folder.

--properties [complex type]

A map of maps that contains the properties which are specific to the
folder type. Each folder type definition defines it’s set of required
...
contained inside of a category. Most folders have required properties
within the “default” category. To determine the set of optional and
required properties for a folder type, a query can be done on
‘/types?type=folder’ that returns a collection of all folder types.
The appropriate folder type, which includes definitions of all of it’s
properties, can be identified from this collection. Example:
...
Type key of the object. Type keys can be found via the ‘/types’
endpoint.
...
--wait-for-state" CREATING "--wait-for-state" MOVING would return on
