...
Modifies the existing OperatorControl for a given operator control id
except the operator control id.
...
--approver-groups-list [complex type]

List of user groups who can approve an access request associated with
a target resource under the governance of this operator control. This
...
--is-fully-pre-approved [boolean]

Whether all the operator actions have been pre-approved. If yes, all
access requests associated with a resource governed by this operator
control will be auto-approved.

--operator-control-id [text]

unique OperatorControl identifier

--operator-control-name [text]
...
--approvers-list [complex type]

List of users who can approve an access request associated with a
target resource under the governance of this operator control. This is
...
--email-id-list [complex type]

List of emailId. This is a complex type whose value must be valid
...
--pre-approved-op-action-list [complex type]

List of pre-approved operator actions. Access requests associated with
a resource governed by this operator control will be automatically
approved if the access request only contain operator actions in the
pre-approved list. This is a complex type whose value must be valid
JSON. The value can be provided as a string on the command line or
passed in as a file using the file://path/to/file syntax.
...
System message that would be displayed to the operator users on
accessing the target resource under the governance of this operator
control.
...
--wait-for-state" CREATED "--wait-for-state" DELETED would return on
