...
rules that specify this *Service* object’s *cidrBlock* as the
destination CIDR. However, consider removing the rules if your intent
is to permanently disable use of the *Service* through this service
gateway.

**Note:** The *DetachServiceId* operation is an easy way to remove an
individual *Service* from the service gateway. Compare it with
UpdateServiceGateway, which replaces the entire existing list of
enabled *Service* objects with the list that you provide in the
*Update* call. *UpdateServiceGateway* also lets you block all traffic
through the service gateway without having to remove each of the
