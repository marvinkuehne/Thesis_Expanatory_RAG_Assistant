...
Backs up an encrypted file that contains all the metadata of a vault
so that you can restore the vault later. You can backup a vault
whether or not it contains keys. This operation only backs up the
...
--is-include-keys [boolean]

A Boolean value that indicates whether the Keys should be included
during backing up the Vault.
