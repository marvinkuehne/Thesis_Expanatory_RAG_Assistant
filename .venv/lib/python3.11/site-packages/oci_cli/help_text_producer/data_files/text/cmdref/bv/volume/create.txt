...
documentation on AutotunePolicy please see our API reference:
https://docs.oracle.com/en-
us/iaas/api/#/en/iaas/20160918/datatypes/AutotunePolicy. This is a
...
--block-volume-replicas [complex type]

The list of block volume replicas to be enabled for this volume in the
specified destination availability domains.

This option is a JSON list with items of type
BlockVolumeReplicaDetails.  For documentation on
BlockVolumeReplicaDetails please see our API reference:
https://docs.oracle.com/en-
us/iaas/api/#/en/iaas/20160918/datatypes/BlockVolumeReplicaDetails.
...
--is-reservations-enabled [boolean]

When set to true, enables SCSI Persistent Reservation (SCSI PR) for
the volume. For more information, see Persistent Reservations.
...
either this, –volume-backup-id or –source-volume-replica-id but not
all. If neither is specified then the new Block volume will be empty.

--source-volume-replica-id [text]

The OCID of the block volume replica from which the data should be
restored on the newly created volume. You can specify either this,
–volume-backup-id or –source-volume-id but not all. If neither is
...
on the newly created volume. You can specify either this, –source-
volume-id or –source-volume-replica-id but not all. If neither is
specified then the new Block volume will be empty.
...
--wait-for-state" PROVISIONING "--wait-for-state" FAULTY would return
...
for the block volume cross region backups, which will be used in the
destination region to encrypt the backup’s encryption keys. For more
information about the Vault service and encryption keys, see Overview
of Vault service and Using Keys.
