...
=====
   ...
   oci data-safe report-definition create [OPTIONS]


Required Parameters
===================

--column-filters [complex type]

An array of column filter objects. A column Filter object stores all
information about a column filter including field name, an operator,
one or more expressions, if the filter is enabled, or if the filter is
hidden. This is a complex type whose value must be valid JSON. The
...
--column-info [complex type]

An array of column objects in the order (left to right) displayed in
the report. A column object stores all information about a column,
including the name displayed on the UI, corresponding field name in
the data source, data type of the column, and column visibility (if
the column is visible to the user). This is a complex type whose value
...
--column-sortings [complex type]

An array of column sorting objects. Each column sorting object stores
the column name to be sorted and if the sorting is in ascending order;
sorting is done by the first column in the array, then by the second
column in the array, etc. This is a complex type whose value must be
...
An array of report summary objects in the order (left to right)
displayed in the report.  A  report summary object stores all
information about summary of report to be displayed, including the
name displayed on UI, the display order, corresponding group by and
count of values, summary visibility (if the summary is visible to
user). This is a complex type whose value must be valid JSON. The
value can be provided as a string on the command line or passed in as
a file using the file://path/to/file syntax.
...
security policies before trying the examples.
       ...
       oci data-safe report-definition create --column-filters file://column-filters.json --column-info file://column-info.json --column-sortings file://column-sortings.json --compartment-id $compartment_id --display-name $display_name --parent-id $parent_id --summary file://summary.json
