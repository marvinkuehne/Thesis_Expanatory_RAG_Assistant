...
*inventoryLog* is now a required parameter for CreateFleet API. Update
existing applications using this API before July 15, 2022 to ensure
the applications continue to work. See the Service Change Notice for
more details. Migrate existing fleets using the *UpdateFleet* API to
set the *inventoryLog* parameter.
...
The name of the Fleet. The displayName must be unique for Fleets in
the same compartment.

--inventory-log [complex type]
...
The Fleet’s description. If nothing is provided, the Fleet description
will be null.
...
--is-advanced-features-enabled [boolean]

Whether or not advanced features are enabled in this Fleet.
Deprecated, use */fleets/{fleetId}/advanceFeatureConfiguration* API
instead.
