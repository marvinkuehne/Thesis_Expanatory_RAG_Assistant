...
The availability domain that the Lustre file system is in. May be
unset as a blank or NULL value.
...
--file-system-path [text]

The path in the Lustre file system used for this Object Storage link.

Example:

   myFileSystem/mount/myDirectory

--is-overwrite [boolean]

The flag is an identifier to tell whether the job run has overwrite
enabled. If *isOverwrite* is false, the file to be imported or
exported will be skipped if it already exists. If *isOverwrite* is
true, the file to be imported or exported will be overwritten if it
already exists.
...
--object-storage-prefix [text]

The Object Storage namespace and bucket name, including optional
object prefix string, to use as the source for imports or destination
for exports.

Example:

   objectStorageNamespace:/bucketName/optionalFolder/optionalPrefix
...
security policies before trying the examples.
       ...
       oci lfs object-storage-link create --availability-domain $availability_domain --compartment-id $compartment_id --file-system-path $file_system_path --is-overwrite $is_overwrite --lustre-file-system-id $lustre_file_system_id --object-storage-prefix $object_storage_prefix
