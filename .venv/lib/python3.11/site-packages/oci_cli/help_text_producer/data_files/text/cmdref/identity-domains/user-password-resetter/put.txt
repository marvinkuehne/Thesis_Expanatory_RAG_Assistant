...
--user-password-resetter-id [text]
...
--bypass-notification [boolean]

Indicates whether to send an email notification after resetting the
user’s password.
...
--notification-email-template-id [text]

Specifies the EmailTemplate to be used when sending notifications to
...
A one time password will be returned when the user doesn’t have a
...
--user-flow-controlled-by-external-client [boolean]

A Boolean value indicating whether to bypass notification and return
user token to be used by an external client to control the user flow.

**SCIM++ Properties:**  - type: boolean  - caseExact: false  -
idcsSearchable: false  - multiValued: false  - mutability: immutable
- required: false  - returned: never  - uniqueness: none

--user-token [complex type]
