...
--ipv4-cidr-blocks [complex type]

The list of all IPv4 CIDR blocks for the subnet that meets the
following criteria: - Ipv4 CIDR blocks must be valid. - Multiple Ipv4
CIDR blocks must not overlap each other or the on-premises network
CIDR block. - The number of prefixes must not exceed the limit of IPv4
prefixes allowed to a subnet. This is a complex type whose value must
...
security policies before trying the examples.
       ...
       oci network subnet create --compartment-id $compartment_id --vcn-id $vcn_id
