...
The key of the connection resource that is used for the harvest by
this job.
...
Interval on which the job will be run. Value is specified as a cron-
supported time specification “nickname”. The following subset of those
is supported: @monthly, @weekly, @daily, @hourly. For metastore sync,
an additional option @default is supported, which will schedule jobs
at a more granular frequency.
...
--wait-for-state" ACTIVE "--wait-for-state" EXPIRED would return on
