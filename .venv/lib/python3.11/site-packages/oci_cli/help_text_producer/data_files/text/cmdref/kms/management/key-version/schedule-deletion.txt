...
lifecycle state of the key version to *PENDING_DELETION* and then
deletes it after the specified retention period ends.
...
An optional property to indicate when to delete the key version,
expressed in RFC 3339 timestamp format. The specified time must be
between 7 and 30 days from the time when the request is received. If
this property is missing, it will be set to 30 days from the time of
