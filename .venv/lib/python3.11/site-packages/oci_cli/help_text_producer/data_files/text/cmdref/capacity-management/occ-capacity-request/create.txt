...
Since all resources are at tenancy level hence this will be the ocid
of the tenancy where operation is to be performed.

--date-expected-capacity-handover [datetime]

The date by which the capacity requested by customers before
dateFinalCustomerOrder needs to be fulfilled.
...
A list of different resources requested by the user. This is a complex
...
An user-friendly name for the capacity request. Does not have to be
unique, and it’s changeable. Avoid entering confidential information.

--namespace [text]

The name of the OCI service in consideration. For example, Compute,
Exadata, and so on.
...
The availability domain (AD) in which the new resource is to be
placed. If this is specified then the capacity will be validated and
fulfilled within the scope of this AD. Note that this field is NOT
...
The OCID of the availability catalog against which capacity request is
made.

--request-state [text]

The subset of request states available for creating the capacity
request.
